## libvterm bindings + high-level terminal wrapper.
## Compiles libvterm 0.3.3 from bundled source in vendor/libvterm.
...
# ── raw bindings (adapted from nitty) ────────────────────────────────────────
...
proc newTerminal*(cols, rows: int32): Terminal =
  result.cols   = cols
  result.rows   = rows
...
proc destroy*(t: var Terminal) =
  if t.vt != nil:
    vterm_free(t.vt)
    t.vt = nil

proc advance*(t: var Terminal; data: openArray[byte]) =
  if data.len > 0:
    discard vterm_input_write(t.vt, cast[ptr char](data[0].unsafeAddr), data.len.uint64)

proc cell*(t: Terminal; row, col: int32): VTermScreenCell =
  discard vterm_screen_get_cell(t.screen, VTermPos(row: row, col: col), addr result)

proc cursorPos*(t: Terminal): VTermPos =
  vterm_state_get_cursorpos(t.state, addr result)

proc resize*(t: var Terminal; cols, rows: int32) =
  t.cols = cols; t.rows = rows
  vterm_set_size(t.vt, rows, cols)

proc sendKey*(t: var Terminal; key: VTermKey; mods = VTermModifier.None) =
  vterm_keyboard_key(t.vt, key, mods)

proc sendChar*(t: var Terminal; c: uint32; mods = VTermModifier.None) =

...
proc main() =
  ...
  block:
    # All Raylib resources live in this block so naylib's =destroy hooks
    # (unloadFont, unloadTexture) fire here — before closeWindow() below.
    ...
    for id in ws.leaves():
      termFree(states[id].trm)
      states[id].pt.close()
    # block exits here: font (and any other Raylib resources) are destroyed
    # by naylib's =destroy hooks while the OpenGL context is still alive
